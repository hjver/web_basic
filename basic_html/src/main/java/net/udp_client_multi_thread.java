...
//UDP - Client
public class udp_client_multi_thread {
...
class data_client_new{
	...
	private void udp_cn() {
		try {
			Thread th_send = new send_to_server(this.ds, this.ia, this.port);
			th_send.start();
			
			Thread th_receive = new receive_from_server(this.ds);
			th_receive.start();
...
class send_to_server extends Thread {
	DatagramSocket ds = null;  //UDP 접속하는 역할
	DatagramPacket dp = null;  //UDP 데이터 송수신 필요한 크기
	InetAddress ia = null; //자신의 IP 정보 및 PORT를 확인
	InputStreamReader isr = null;
	...
	public send_to_server(DatagramSocket ds, InetAddress ia, int port) {
		this.ds = ds;
		this.ia = ia;
		this.port = port;
	}
...
class receive_from_server extends Thread {
	DatagramSocket ds = null;  //UDP 접속하는 역할
	DatagramPacket dp = null;  //UDP 데이터 송수신 필요한 크기
	
	public receive_from_server(DatagramSocket ds) {
		this.ds = ds;
	}
